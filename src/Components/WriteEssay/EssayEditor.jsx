...
const TextEditor = forwardRef(({ refText, onSave,initialvalue, onSubmit, userGroup }, ref) => {
  ...
  const updateContent = () => {
    if (editableContentRef.current) {
      ...
      const result = getVerbatim(refText, content, userGroup);
      ...
      if (editableText !== updatedContent) {
        ...
        verbatimFlaggedWords.push(...result.verbatiFlaggedWords)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex justify-between bg-gray-200 p-2 border-b border-gray-300 dark:bg-gray-700 dark:border-gray-600">
        <button
          onClick={() => onSave(editableText,[...new Set(verbatimFlaggedWords)])}
          className="bg-green-500 hover:bg-green-600 text-white p-2 rounded"
        >
          Save
        </button>
        <button
          onClick={() => onSubmit(editableText,[...new Set(verbatimFlaggedWords)])}
